...
```{r}
porzadek <- porzadek %>%
  mutate(st85q01 = case_when(st85q01 %in% c(1, 2) ~ 1,
                             st85q01 %in% c(3, 4) ~ 0),
         st85q02 = case_when(st85q02 %in% c(1, 2) ~ 1,
                             st85q02 %in% c(3, 4) ~ 0),
         st85q03 = case_when(st85q03 %in% c(1, 2) ~ 1,
                             st85q03 %in% c(3, 4) ~ 0),
         st85q04 = case_when(st85q04 %in% c(3, 4) ~ 1,
                             st85q04 %in% c(1, 2) ~ 0))
```
...
# podejście z rowSums
dyscyplina <- dyscyplina %>%
  mutate(sameBD1 = rowSums(cur_data() %>%
                            select(starts_with("st81q")) %>%
                            is.na()) == 5)
# podejście z rowwise()
dyscyplina <- dyscyplina %>%
  rowwise() %>%
  mutate(sameBD2 = sum(is.na(c_across(starts_with("st81q")))) == 5) %>%
  ungroup()
```
...
przynaleznosc <- przynaleznosc %>%
  mutate(irv = irv(cur_data() %>% select(starts_with("st87"))),
         longstring = longstring(cur_data() %>% select(starts_with("st87"))))
```
...
przynaleznosc <- przynaleznosc %>%
  rowwise() %>%
  mutate(skrajne = sum(c_across(starts_with("st87")) %in% c(1, 4)) / 9,
         tak_wprost =
           sum(c_across(c(starts_with("st87"),
                          -st87q01, -st87q04, -st87q06)) %in% c(1, 2)) / 6,
         tak_odwr = sum(c_across(c(st87q01, st87q04, st87q06)) %in% c(1, 2)) / 3,
         irv_ja = sd(c_across(starts_with("st87"))))
```
...
```{r}
# to jest miejsce na Twój kod
przynaleznoscWersja <- przynaleznosc %>%
  group_by(questid) %>%
  summarise(irv = mean(irv, na.rm = TRUE),
            longstring = mean(longstring, na.rm = TRUE),
            skrajne = mean(skrajne, na.rm = TRUE),
            tak_wprost = mean(tak_wprost, na.rm = TRUE),
            tak_odwr = mean(tak_odwr, na.rm = TRUE))
View(przynaleznoscWersja)

przynaleznoscWersjaKraj <- przynaleznosc %>%
  group_by(cnt, questid) %>%
  summarise(irv = mean(irv, na.rm = TRUE),
            longstring = mean(longstring, na.rm = TRUE),
            skrajne = mean(skrajne, na.rm = TRUE),
            tak_wprost = mean(tak_wprost, na.rm = TRUE),
            tak_odwr = mean(tak_odwr, na.rm = TRUE))
View(przynaleznoscWersjaKraj)
```
...
przynaleznosc %>%
  group_by(questid) %>%
  summarise(across(c(longstring, skrajne),
                   list(srednia = ~mean(., na.rm = TRUE),
                        odch.std. = ~sd(., na.rm = TRUE))))
```
